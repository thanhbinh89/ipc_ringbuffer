...
public:
    ...
    T dump_front();
    T getNodeAt(size_t index) const;
...
private:
    ...
    // Guard to ensure read_unlock is always called
    class ReadUnlockGuard {
    public:
        ReadUnlockGuard(ReadWriteLock &lock) : _lock(lock), _active(true) {
            _lock.read_lock();
        }
        ~ReadUnlockGuard() {
            if (_active) _lock.read_unlock();
        }
        void release() { _active = false; } // Optional, to release manually if needed
    private:
        ReadWriteLock &_lock;
        bool _active;
    };

     // Guard to ensure write_unlock is always called
    class WriteUnlockGuard {
    public:
        WriteUnlockGuard(ReadWriteLock &lock) : _lock(lock), _active(true) {
            _lock.write_lock();
        }
        ~WriteUnlockGuard() {
            if (_active) _lock.write_unlock();
        }
        void release() { _active = false; } // Optional, to release manually if needed
    private:
        ReadWriteLock &_lock;
...
template <typename T> inline size_t
ShmRingBuffer<T>::available() const
{
    assert(_hdr != NULL);

    size_t avai = 0;
    ReadUnlockGuard guard(*_lock);
    avai = (_hdr->_end + _hdr->_capacity - _hdr->_begin) % _hdr->_capacity;
    return avai;
}
...
template <typename T> inline T
ShmRingBuffer<T>::getNodeAt(size_t index) const
{
    assert(_hdr != NULL);
    assert(_v != NULL);
    ...
    T ret;
    WriteUnlockGuard guard(*_lock);
    size_t avai = (_hdr->_end + _hdr->_capacity - _hdr->_begin) % _hdr->_capacity;
    if (index >= avai) {
        throw std::out_of_range("Index out of range");
    }
    size_t actualIndex = (_hdr->_capacity + _hdr->_end - 1 - index) % _hdr->_capacity;
    ret = *(_v + actualIndex);
    return ret;
}
